define(['jquery', 'underscore', 'mustache', 'modallica', 'buzzkill', 'collections/chart', 'collections/workbook', 'views/base', 'charts/charts', 'views/worksheet/_create-worksheet'],
function($, _, Mustache, Modallica, Buzzkill, ChartCollection, WorkbookCollection, View, Trace, CreateWorksheet) {
    'use strict';
    ...
    var ChartView = View.extend({
        ...
        setLabelsPosition: function($chart) {
            if ($chart.find('.trace-legend .label').length > 1) {
                ...
                var width = (this.traceConfig.settings.chart.width - this.margin[3] - this.margin[1]);
        ...
        getFinalTraceConfig: function($chart, chartData) {
            ...
            this.margin = [40, 20, 20, characterLimit * letterWidth];
            ...
            config = {
                ...
                tooltips: function() {
                    ...
                    if (this.data.comparison === true) {
                        tooltip += ' (' + Math.abs(args.baseline) + ')';
                    } else if (this.data.comparison === false) {
                        tooltip += ' (' + Math.abs(args.value / args.baseline) + ')';
                    }
                ...
                yTickFormat: function(label) {
                    ...
                    if (label.toString().length > characterLimit) {
                        this.setAttribute('data-tooltip', label);
                        return label.toString().substr(0, characterLimit - 3) + '...';
                    } else if (label.toString().indexOf('http') !== -1) {
                        this.setAttribute('data-tooltip', label);
                    }
                    return label;
                },
