...
function($, Mustache, Modallica, selectize, Buzzkill, RecordingCollection, WorkbookCollection, ScrollFoo, Router, View) {
    ...
    var ProjectWorkbookSidebarView = View.extend({
        ...
        templateEl: '.create-workbook__select--template',
        templateDescriptionEl: '.create-workbook__template-description p',
        ...
        bindEvents: function() {
            ...
            $('body').on('change.workbooksidebar', this.templateEl, function() {
                var templateVal = $(this).val();
                _this.dataObject.template = templateVal;
            ...
            $(this.el).find('[data-workbook-action="edit"]').on('click', function (evt) {
            ...
            // when we click the menu option make sure we recalculate so the 
            // menu doesn't dissapear
            $(this.el).find('span[data-dropdown-toggle]').on('click', function () {
                setTimeout(function () {
        ...
        /**
         * Fetches the Workbook template collection
         */
        getTemplates: function() {
            var _this = this;

            return new Promise(function(resolve, reject) {
                $.get('/api/project/' + _this.data.project.id + '/workbook/templates')
        ...
        doCreateWorkbook: function() {
            ...
            this.loader.showLoader(
                $('[data-modallica-template-name="workbook-create-modal"]'), 
                'Creating workbook, this could take up to 45 seconds'
            );
         ...
         * @return {Object}          View instance
         */
        renderTemplates: function(viewTemplate) {
            var templateHtml = '';
            _.each(this.templates, function(template) {
                templateHtml += Mustache.render(viewTemplate, {
                    id: template.id,
                    name: template.title,
            ...
            $(this.templateEl).append(templateHtml);

            $(this.templateEl).selectize({
                render: {
            ...
            this.getRecordings()
                .then(function() {
                    ...
                    this.getTemplates().then(function() {
                        this.renderTemplates(selectOptionTemplate);
