define([
...
], function ($, _, Backbone, moment, Calendar, ErrorHandler, CalendarTemplate) {
	...
	var CalendarSidebarView = Backbone.View.extend({
		...
		render: function () {
			...
			// watch for scrolling
			$('.scrollfoo__parent').on('scroll', function () {
				this.position();
			}.bind(this));
		 ...
		 *
		 * We are creating a new element in the document, not a child of the
		 * current element. Therefore we need to work out the position of the
		 * should be parent and position the new element next to it.
		 * 
		 * @return {[type]} [description]
		 */
		position: function () {
			...
			this.$el.css({
				'top': position.top + 34 - document.body.scrollTop + 'px',
				'left': position.left + 303 + 'px'
			});
		},
		...
		change: function () {
			...
			if (!start.isValid() && !this.fromValidError) {
				// throw an error
				...
				return;
			} else if (this.fromValidError) {
				this.fromValidError = this.fromValidError.remove();
			}

			// if it's less than the min
			if (start < this.min && !this.fromLessError) {
				// throw an error
				this.$el.find('.controls .from').addClass('errored');
				this.$el.find('.submit-button').addClass('disabled');
				this.fromLessError = new ErrorHandler('The start date cannot be before ' + moment(this.min).format('YYYY-MM-DD HH:MM'))
					.tooltip(this.$el.find('input[name="from-date"]')[0], 'top');
				return;
			} else if (this.fromLessError) {
				this.fromLessError = this.fromLessError.remove();
			}
			...
			if (start > this.max && !this.fromGreaterError) {
				...
				this.fromGreaterError = new ErrorHandler('The start date cannot be after ' + moment(this.max).format('YYYY-MM-DD HH:MM'))
					.tooltip(this.$el.find('input[name="from-date"]')[0], 'top');
				return;
			} else if (this.fromGreaterError) {
				this.fromGreaterError = this.fromGreaterError.remove();
			}
			...
			if (!end.isValid() && !this.toValidError) {
				...
				return;
			} else if (this.toValidError) {
				this.toValidError = this.toValidError.remove();
			}
			...
			if (end < this.min && !this.toLessError) {
				...
				this.toLessError = new ErrorHandler('The end date cannot be before ' + moment(this.min).format('YYYY-MM-DD HH:MM'))
					.tooltip(this.$el.find('input[name="to-date"]')[0], 'top');
				return;
			} else if (this.toLessError) {
				this.toLessError = this.toLessError.remove();
			}
			...
			if (end > this.max && !this.toGreaterError) {
				...
				this.toGreaterError = new ErrorHandler('The end date cannot be after ' + moment(this.max).format('YYYY-MM-DD HH:MM'))
					.tooltip(this.$el.find('input[name="to-date"]')[0], 'top');
				return;
			} else if (this.toGreaterError) {
				this.toGreaterError = this.toGreaterError.remove();
			}
