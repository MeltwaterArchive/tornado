...
define([
	...
	/**
	 * Returns the tooltip element. Creates one first, if necessary
	 */
	Trace.prototype._getTooltip = function(evt) {
		var existingTooltip = document.querySelectorAll('.trace-tooltip');
		...
		if (existingTooltip.length === 0) {
			...
			this.tooltip = existingTooltip[0];
		}
	...
	Trace.prototype._mouseover = function (evt) {
		...
		if (this.options.tooltips) {
			this.tooltip = this._getTooltip(evt);

			var tooltipWidth = this.tooltip.offsetWidth,
				tooltipHeight = this.tooltip.offsetHeight,
				elementRect = d3.event.target.getBoundingClientRect();

			this.tooltip.style.left = (elementRect.left + document.body.scrollLeft + (elementRect.width / 2) - (tooltipWidth / 2)) - 12 + 'px';
			this.tooltip.style.top = (elementRect.top + document.body.scrollTop - (tooltipHeight * 2) - 25) + 'px';

			// Allow for an extra browser tick, before we show the tooltip
			setTimeout(function() {
				this.tooltip.className = 'trace-tooltip trace-tooltip--active';
			}.bind(this), 0);
		}
	};
